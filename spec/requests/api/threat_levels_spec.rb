...
describe Api::V1::ThreatLevelsController do
  ...
  describe 'GET /threat_levels' do
    ...
    context 'profile present in DB' do
      ...
      it 'returns profile without data' do
        get api("/threat_levels", user)
        expect(json_response).to eq(threat_levels_with_profile_only)
      end
      ...
      context 'threat level data present in DB' do
        ...
        context '6 assessments exist in DB' do
          before do
            5.times { create(:threat_assessment, profiles: [profile]) }
          end
...
def threat_levels_with_profile_only
  {
    'threat_levels' => [
      {
...
def threat_levels_with_results
  threat_levels = threat_levels_with_profile_only
